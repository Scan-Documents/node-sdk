...
import { File, FileListParams, FileListResponse, FileUploadParams, Files } from './resources/files';
import {
  ApplyEffectResponse,
  BoundingBox,
  ConvertRequest,
  ConvertResponse,
  CoordinatesItem,
  DetectDocumentsResponse,
  ExtractTextRequest,
  ExtractTextResponse,
  ImageFromTaskResponse,
  ImageOperationApplyEffectParams,
...
import {
  ...
  ExtractPages,
  Merge,
  PdfOperationExtractPagesParams,
  PdfOperationMergeParams,
  PdfOperationRenderParams,
  PdfOperationSplitParams,
  PdfOperations,
  Render,
  Split,
} from './resources/pdf-operations';
import { TaskListParams, TaskListResponse, TaskResponse, Tasks } from './resources/tasks';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['SCAN_DOCUMENTS_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SCAN_DOCUMENTS_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Scan Documents API.
 */
export class ScanDocuments {
  apiKey: string;
   ...
   * API Client for interfacing with the Scan Documents API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['SCAN_DOCUMENTS_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['SCAN_DOCUMENTS_BASE_URL'] ?? https://api.scan-documents.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('SCAN_DOCUMENTS_BASE_URL'),
    apiKey = readEnv('SCAN_DOCUMENTS_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.ScanDocumentsError(
        "The SCAN_DOCUMENTS_API_KEY environment variable is missing or empty; either provide it, or instantiate the ScanDocuments client with an apiKey option, like new ScanDocuments({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.scan-documents.com`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? ScanDocuments.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SCAN_DOCUMENTS_LOG'), "process.env['SCAN_DOCUMENTS_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.scan-documents.com';
  }
  ...
  static ScanDocuments = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static ScanDocumentsError = Errors.ScanDocumentsError;
...
export declare namespace ScanDocuments {
  ...
  export {
    ImageOperations as ImageOperations,
  ...
  export {
    PdfOperations as PdfOperations,
