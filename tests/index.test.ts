...
import { APIPromise } from 'scan-documents/core/api-promise';

import util from 'node:util';
import ScanDocuments from 'scan-documents';
import { APIUserAbortError } from 'scan-documents';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['SCAN_DOCUMENTS_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: ScanDocuments) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new ScanDocuments({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new ScanDocuments({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'SCAN_DOCUMENTS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['SCAN_DOCUMENTS_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new ScanDocuments({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['SCAN_DOCUMENTS_BASE_URL'] = 'https://example.com/from_env';
      const client = new ScanDocuments({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['SCAN_DOCUMENTS_BASE_URL'] = ''; // empty
      const client = new ScanDocuments({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.scan-documents.com');
    });

    test('blank env variable', () => {
      process.env['SCAN_DOCUMENTS_BASE_URL'] = '  '; // blank
      const client = new ScanDocuments({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.scan-documents.com');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new ScanDocuments({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['SCAN_DOCUMENTS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new ScanDocuments({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new ScanDocuments({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new ScanDocuments({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['SCAN_DOCUMENTS_API_KEY'] = 'My API Key';
    const client = new ScanDocuments();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['SCAN_DOCUMENTS_API_KEY'] = 'another My API Key';
    const client = new ScanDocuments({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new ScanDocuments({ apiKey: 'My API Key', fetch: testFetch });
